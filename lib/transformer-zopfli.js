...
module.exports = Transformer.extend({
	...
	compress: function(content, destPath, options) {
		var encoding = options.encoding ? options.encoding.read : null;
		...
		try {
			...
			var result = zopfli.gzipSync(new Buffer(content, encoding), options);
		...
		return srcCollection.forEachSeries(co.wrap(function *(srcFile, index, done) {
			try {
				...
				options = extend(options, {
					encoding: srcCollection.getOptions().encoding
				});
